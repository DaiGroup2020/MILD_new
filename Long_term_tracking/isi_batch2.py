...
def load_isi_hist(recording_time):
    isi_path=os.path.join(root_folder_input,experimenter,acquisition_system,mouse_id,recording_time,"analysis","sorting_result","waveform_from_phy","isi_histograms","isi_histograms.npy")
    if not os.path.exists(isi_path):
        print("Missing:",isi_path)
        return None
    hist_all=np.asarray(np.load(isi_path))
    row_sum=np.sum(hist_all,axis=1,keepdims=True)
    row_sum[row_sum==0]=1
    hist_all=hist_all/row_sum
    return hist_all


def get_unit_isi(recording_time,unit_name):
    ...
    if position is None:
        return None
    hist_all=load_isi_hist(recording_time)
    if hist_all is None or position>=hist_all.shape[0]:
        return None
    return hist_all[position,:]
...
for unit_index,unit_name in enumerate(unit_names):
    ...
    for recording_time,month in time_mapping.items():
        ...
        z=get_unit_isi(recording_time,unit_name)
    ...
    ax.set_zlim(0,Z_MAX)
    ax.set_zticks(np.linspace(0,Z_MAX,3))
    ...
    save_path=os.path.join(save_dir,f"cellbundle-isi_{unit_name}.svg")
